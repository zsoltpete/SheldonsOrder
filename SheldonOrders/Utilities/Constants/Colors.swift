//
//  Colors.swift
//  SheldonOrders
//
//  Created by Zsolt Pete on 2018. 12. 05..
//  Copyright © 2018. Zsolt Pete. All rights reserved.
//

import Foundation
import UIKit

struct Colors {
    
    struct Login {
        
        static let MainTitle = UIColor.black
        static let LoginItemTitle = UIColor.black
        static let LoginItemBottomBorder = UIColor(red: 151/255, green: 151/255, blue: 151/255, alpha: 1.0)
        static let LoginButtonTitle = UIColor.white
        static let LoginButtonBackground = UIColor(red: 74/255, green: 144/255, blue: 226/255, alpha: 1.0)
        static let LogiButtonShadow = UIColor(red: 0/255, green: 0/255, blue: 0/255, alpha: 0.5)
    }
    
    struct TypeSelectionCell {
        static let Title = UIColor.black
        static let OrderButton = UIColor(red: 97/255, green: 189/255, blue: 235/255, alpha: 1.0)
        static let OrderedButton = UIColor(red: 198/255, green: 198/255, blue: 198/255, alpha: 1.0)
        static let Seperator = UIColor(red: 151/255, green: 151/255, blue: 151/255, alpha: 1.0)
        static let Description = UIColor.black
        static let Price = UIColor(red: 196/255, green: 81/255, blue: 81/255, alpha: 1.0)
        static let Background = UIColor.white
    }
    
    struct InvoiceIntentView {
        static let Title = UIColor.black
        static let Price = TypeSelectionCell.Price
    }
    
    struct ConfirmIntentView {
        static let Title = UIColor.black
    }
    
    struct MealTypeCell {
        static let Background = UIColor.white
    }
    
}
